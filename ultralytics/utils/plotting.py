...
class Annotator:
    ...
    def queue_counts_display(
        ...
        text_size = cv2.getTextSize(
            label, cv2.FONT_HERSHEY_SIMPLEX, fontScale=fontsize, thickness=self.tf
        )[0]
    ...
    def display_counts(
        ...
        t_sizes = [
            cv2.getTextSize(str(count), 0, fontScale=0.8, thickness=tf)[0]
            for count in counts
        ]
        ...
        for i, count in enumerate(counts):
            ...
            cv2.line(
                self.im,
                (text_x_pos, line_y_pos),
                (text_x_pos + max_text_width, line_y_pos),
                line_color,
                tf,
            )
